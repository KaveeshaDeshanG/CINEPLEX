...
if (isset($_GET['id'])) {
    ...
    // Fetch movie details
    $query = "SELECT * FROM cmovies WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE cmovies SET name = ?, year = ?, discrip = ?, theater = ?, img = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sssssi", $name, $year, $discrip, $theater, $imagePath, $id);

    if ($stmt->execute()) {
        echo "<script>alert('Movie updated successfully!');</script>";
        header("Location: coming.php"); // Redirect back to the movie list
        exit;
    } else {
